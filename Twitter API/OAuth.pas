...
uses
  Classes, SysUtils, IdURI, Windows,dialogs, Generics.Collections,synacode,idCoder3To4;
...
begin
  ...
  begin
    begin
      ...
      begin
        if encodeHttp then
          buf := Params.Names[i] + '='+ Utf8Encode(Params.ValueFromIndex[i]) + ''
        else
...
begin
  ...
  signature := EncodeBase64(HMAC_SHA1(base, consec));

  oAuthParams.Add('oauth_signature=' + UrlEncode(signature));
  Result := 'OAuth ' + encodeParams(oAuthParams,',',true);
  oParams.Free;
  oAuthParams.Free;
end;
